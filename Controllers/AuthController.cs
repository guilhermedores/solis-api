...
public class AuthController : ControllerBase
{
    ...
    public async Task<IActionResult> Login([FromBody] LoginRequest request)
    {
        ...
        if (string.IsNullOrWhiteSpace(tenantSubdomain))
            return BadRequest(new ErrorResponse { Error = "Header X-Tenant-Subdomain é obrigatório" });

        if (string.IsNullOrWhiteSpace(request.Email) || string.IsNullOrWhiteSpace(request.Password))
            return BadRequest(new ErrorResponse { Error = "Email e senha são obrigatórios" });

        var result = await _authService.LoginAsync(tenantSubdomain, request);

        if (result == null)
            return Unauthorized(new ErrorResponse { Error = "Credenciais inválidas ou tenant não encontrado" });
    ...
    public async Task<IActionResult> Me()
    {
        ...
        if (string.IsNullOrWhiteSpace(authHeader) || !authHeader.StartsWith("Bearer "))
            return Unauthorized(new ErrorResponse { Error = "Token de autenticação não fornecido" });

        var token = authHeader.Substring("Bearer ".Length).Trim();

        var payload = _authService.ValidateToken(token);
        if (payload == null)
            return Unauthorized(new ErrorResponse { Error = "Token inválido ou expirado" });

        var result = await _authService.GetUserFromTokenAsync(token, payload.Tenant);

        if (result == null)
            return Unauthorized(new ErrorResponse { Error = "Token inválido ou usuário não encontrado" });
    ...
    public async Task<IActionResult> GenerateAgentToken([FromBody] GenerateAgentTokenRequest request)
    {
        ...
        if (request.StoreId == Guid.Empty)
            return BadRequest(new ErrorResponse { Error = "StoreId é obrigatório" });

        if (string.IsNullOrWhiteSpace(request.AgentName))
            return BadRequest(new ErrorResponse { Error = "AgentName é obrigatório" });

        var tenant = await _authService.GetTenantBySubdomainAsync(tenantSubdomain);
        if (tenant == null)
            return NotFound(new ErrorResponse { Error = "Tenant não encontrado" });

        var storeExists = await _authService.VerifyStoreExistsAsync(tenantSubdomain, request.StoreId);
        if (!storeExists)
            return NotFound(new ErrorResponse { Error = "Loja não encontrada no tenant especificado" });
